...
### Satellite basemap (2026-09-16, ported from ts-mission-builder)

Optional second pyramid per map — `MapConfig.sat = {tilePattern, tileMaxZoom, nativeZoom}` — toggled by the "SAT" HUD button (`MapViewControls`, rendered only when the map has `sat`; preference in localStorage `ts-ops-planner-sat-v1`, not part of the plan). 2D = a second `<TileLayer zIndex={2}>` in MapClient.tsx above the topo layer; 3D = `compositeTerrainTexture(cfg, sat)` (cache keyed per mode) plus an in-place material-map swap effect in MapClient3D.tsx (setup reads `satLayerRef` for the initial texture). **Scale contract**: the topo pyramids are 1 px = 1 m at their deepest level (Leaflet zoom 0), so a sharper satellite pyramid must be an exact power of two px/m — `nativeZoom` = log2(px/m); the layer's `zoomOffset` = tileMaxZoom − nativeZoom, `maxNativeZoom` = nativeZoom. Imagery = EnfusionMapMaker nadir World Editor captures stitched by `tools/map-gen/nadir_mosaic.py` (ortho mode against the Builder heightmap; measure the per-session yaw with `nadir_orient.py` first) → `tile_pyramid.py`; the pyramids are byte-identical copies of the Builder's `web/public/tiles/<key>-sat` (Kolguyev `tileMaxZoom 8`, 317 MB; Novka `tileMaxZoom 6`, 45 MB — both 4 px/m, `nativeZoom 2`). Capture recipe, measurements and the step 100 / 1200 m decision live in the Builder's CLAUDE.md "Satellite basemap" section and the `EnfusionMapMaker/CAPTURE.md` write-up; add a map here by copying its `-sat` folder and the `sat` line from the Builder's terrains.ts.
...
## 3D view — plan + replay (2026-08-02)

Both modes have a 3D viewport (`web/src/components/MapClient3D.tsx`, raw three.js + CSS2DRenderer — no OrbitControls/fiber). Toggle = "3D" button in the shared top-right HUD cluster (`MapViewControls.tsx`, rendered by both viewports; replaces the old Leaflet ZoomControl and the old replay-only 2D/3D pill). Desktop-only — mobile force-drops to 2D via matchMedia. Lineage note: ts-mission-builder's `MissionMap3D` was forked FROM this file, grew the editing core, and that core was ported back here — keep the two in sync when fixing drape/pick bugs.

- **Swap strategy (page.tsx)**: the 2D Leaflet map stays MOUNTED inside a `hidden` wrapper (its pan/zoom survives the round-trip); 3D mounts on demand. Per-view `mapApi2d`/`mapApi3d` refs + `view3DRef` for non-React event paths; entering 3D hands the viewport over via `MapApi.getView()` → `initialView` (consumed once per setup, bounds-checked against the world). Hidden-map guards in `MapClient`: `FitWorld` skips fitting at size 0 (and completes the deferred fit on reveal via its ResizeObserver), `MapFocusEffect` ignores focus requests while hidden, `getView()` returns null at size 0 — a 0-size Leaflet map reports a garbage max-zoom view that would otherwise poison the handoff.
- **Terrain** = one BufferGeometry (segment cap 256×256) sampled from the heightmap, UNLIT `MeshBasicMaterial` — the map tiles carry baked hillshading; scene lights would double-shade slopes. Texture = the XYZ tile pyramid stitched at runtime into a ≤6144px canvas (`lib/tileComposite.ts`, LRU cache of 3); single-JPG fallback exists only for maps without tiles. Don't regress to loading `<name>_final.jpg` in 3D — Zimnitrita's decodes to ~1 GB RGBA.
- **Drape/pick core**: `MeshGrid` records the exact rendered vertex grid; `meshY()` interpolates it triangle-exact; every draped overlay (lines, fills, ruler) rides `meshY + lift` (LINE_LIFT 1.5 / FILL_LIFT 0.75 / ICON_LIFT 2) so nothing pokes through the decimated mesh. `pickTerrain()` = analytic ray-march over that grid with bisection — cheap enough for pointermove; used for clicks, drags, MMB/wheel pivots, and the hover grid+elevation readout.
- **Plan content in 3D**: markers = CSS2DObjects reusing the exact 2D DivIcon HTML (`markerDivIconHtml`/`militaryDivIconHtml`); lines = `Line2`/`LineMaterial` with `worldUnits: true` → widths in real world meters (truer to `widthM` than the 2D px approximation), resampled at half the terrain cell so they follow ridges; polygons = ear-clip triangulation + midpoint subdivision down to cell size; `fillOutside` zones render outline-only (an exterior mask doesn't drape).
- **Editing works in 3D**: click-to-place markers, line drafting (click vertices / dblclick commit / RMB-or-Escape cancel; the rubber-band lives in a separate `syncDraft` group so mousemove never rebuilds the plan layer), marker select/drag (`makeDraggable` captures pointer events on the CSS2D element, so camera pan is suppressed structurally), duplicate/delete chips, line select (screen-space pick, 14px threshold — 2D hit-polyline parity), ruler line mode incl. the LOS profile panel. Still 2D-only: radial LOS (arming it force-switches to 2D, same pattern as mission-builder's sector draw), the on-map rotation handle (the panel's numeric field works in 3D), polygon `fillOutside` shading.
- **Directional markers** (`lib/directionalMarkers.ts` — hand-maintained because `markerLibrary.ts` is generated): SBF/ABF/AOA-* treat `rotation` as a WORLD bearing; `render()` re-projects each one's bearing vector to a screen angle every frame and rewrites the icon's CSS rotation. Every other icon and all military markers stay screen-upright with rotation baked, exactly like 2D.
- **3D gotchas**: (1) the shared marker HTML self-centers via `translate(-50%,-50%)` (built for Leaflet's iconAnchor `[0,0]`) and CSS2DRenderer ALSO centers its host element — force the host to 0×0 (`planCss2dNode`) so the two don't stack; (2) an in-flow div inside that 0×0 host computes width 0, so label pills need `display:inline-block` (or an explicit width) or their text overflows the box; (3) `LineMaterial` needs `resolution` set at creation AND refreshed on resize; (4) the setup effect is keyed on `mapConfig` only, so interaction callbacks flow through a per-render-refreshed `cbRef` — raw props inside the listeners would go stale.
